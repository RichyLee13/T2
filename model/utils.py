...
class TestSetLoader(Dataset):
    ...
    def _testval_sync_transform(self, img, mask):
        ...
        img, mask = np.array(img), np.array(mask,
                                            dtype=np.float32)  # images: <class 'mxnet.ndarray.ndarray.NDArray'> (512, 512, 3)
        return img, mask
...
def split_evaluation(dataset_dir, original_mask_dir, target_image_path, save_train_result_dir, base_size, FA, PD,
                     supervision, split_method):
...
def split_test_evaluation(dataset_dir, original_mask_dir, target_image_path, save_train_result_dir, base_size, FA, PD,
                          mean_IOU, st_model):
    if 'NUDT-SIRST' in save_train_result_dir:
        evaluation_mode = ['test_point', 'test_spot', 'test_spot_tiny', 'test_spot_small', 'test_spot_medium',
                           'test_spot_big', 'test_extended']
    elif 'NUAA-SIRST' in save_train_result_dir:
        evaluation_mode = ['test_point', 'test_spot', 'test_extended']
    elif 'IRSTD-SIRST' in save_train_result_dir:
        evaluation_mode = ['test_point', 'test_spot', 'test_extended']

    with open('/media/gfkd/software/SIRST_Detection/WS_SIRST/result_WS/' + st_model + '/' + save_train_result_dir,
              'a') as f:
        f.write("FA:")
        f.write(str(FA))
        f.write("\n")
    ...
    for item in range(len(evaluation_mode)):
        ...
        with open(
                '/media/gfkd/software/SIRST_Detection/WS_SIRST/' + 'result_WS' + '/' + st_model + '/' + save_train_result_dir,
                'a') as f:
            f.write("mIoU:")
            f.write(mode)
            f.write(str(total_inter / total_union))
...
def save_resize_pred(pred, size, crop_size, target_image_path, val_img_ids, num, suffix):
    preds = np.array((pred > -40 ).cpu()).astype('int64') * 255
